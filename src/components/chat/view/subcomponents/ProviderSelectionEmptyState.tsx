...
export default function ProviderSelectionEmptyState({
  ...
  const singleProvider = enabledProvidersList.length === 1;

  // Auto-select the only enabled provider so the user can start typing immediately
  React.useEffect(() => {
    if (singleProvider && provider !== enabledProvidersList[0].id) {
      selectProvider(enabledProvidersList[0].id);
    }
  }, [singleProvider]);
    ...
    return (
      <div className="flex h-full items-center justify-center px-4">
        <div className="w-full max-w-md">
          ...
          {/* Model picker — appears after provider is chosen (hidden for single provider) */}
          {!singleProvider && (
            <div
          ...
          {/* Single provider — simple ready prompt */}
          {singleProvider && (
            <p className="text-center text-sm text-muted-foreground/70">
              {
                {
